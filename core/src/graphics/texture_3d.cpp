...
Texture3D::Texture3D(std::array<std::string, 6> paths, GLenum textureWrapS,
...
void Texture3D::loadTextureFace(GLenum face, std::string path) {
    ...
    if (data) {
        this->height = height;
        this->width = width;
        this->setupTextureFace(face, nrChannels == 4 ? GL_RGBA : GL_RGB, width,
                               height, data);
    } else {
...
void Texture3D::setupTextureFace(GLenum face, GLenum type, int width,
    ...
    glTexImage2D(face, 0, type, width, height, 0, type, GL_UNSIGNED_BYTE, data);
};
